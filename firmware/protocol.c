...
void protocol_execute_runtime()
{
  ...
  if( bit_is_clear( DEPTH_PROBE_PIN , DEPTH_PROBE_BIT ) == 1 ){   // check if low 
	  delay_ms(300);
	  if( bit_is_clear( DEPTH_PROBE_PIN , DEPTH_PROBE_BIT ) == 1 ){   // Re-check if low 
  //if( bit_is_clear( PINC , PINC0 ) == 1 ){   // check if low 
		  printPgmString(PSTR("depth\r\n"));
		  delay_ms(250);
	  }
  }
}  
